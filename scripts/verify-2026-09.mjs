...
//
// Verificación del sitio (encargo internacional 2026-09) en navegador real (Chromium vía Playwright).
...
//
//   OUT_DIR=docs/capturas/2026-09/fase-1 node scripts/verify-2026-09.mjs [baseUrl]
import { chromium } from "@playwright/test";
import { mkdir } from "node:fs/promises";
import path from "node:path";

const BASE = process.argv[2] || "http://127.0.0.1:3210";
const OUT = process.env.OUT_DIR || "docs/capturas/2026-09/fase-1";

// Páginas a verificar: lista por defecto de la FASE 1, o las que se pasen
// en PAGES_JSON (array de {path,name,expect404?}).
const PAGES = process.env.PAGES_JSON
  ? JSON.parse(process.env.PAGES_JSON)
  : [
      { path: "/", name: "home-es" },
...
// Enlaces internos que todavía no existen (se construyen en una fase
// posterior): ALLOW_404 lista rutas separadas por comas.
const ALLOW_404 = (process.env.ALLOW_404 || "").split(",").filter(Boolean);
  ...
  for (const page of PAGES) {
    ...
    if (vp.name === "1440" && !page.expect404) {
      ...
      for (const href of internal) {
        ...
        if (status !== 200 && !ALLOW_404.includes(href)) {
